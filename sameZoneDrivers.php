...
if (isset($_GET['employee_id']) && !empty($_GET['employee_id'])) {
    ...
    if($db->fetchFromUsersAccessType($employee_id)==="passenger"){
      $result = $db->getSameZoneDrivers($employee_id);
    ...
    if($result){
       ...
       $response["message"] = "Successful response";
        ...
        {
            //fetching employee_id, employee_name, address, designation, owns_vehicle, mobile_number, gender 
    ...
    echo json_encode($response);
            ...
            $response["error"]["error_msg"] = "No Content : Sorry! No Drivers in Your Zone";
            ...
            $response["error"]["error_msg"] = "Credentials are wrong : Not a Passenger";
            $response["response"] = (object)array();
            echo json_encode($response);
        }
    }
